...
from ta_job_queue import TATaskQueue, TAJob
...
# ── DEX module — deferred import to reduce boot RSS/OOM ──────────────────────
_DEX_BUILD_TOP10 = None
DEX_MODULE_AVAILABLE = False
_DEX_IMPORT_ERROR = None

def _ensure_dex_loaded() -> bool:
    global _DEX_BUILD_TOP10, DEX_MODULE_AVAILABLE, _DEX_IMPORT_ERROR
    if _DEX_BUILD_TOP10 is not None:
        DEX_MODULE_AVAILABLE = True
        return True
    _rss_before = _process_rss_mb()
    try:
        _DEX_BUILD_TOP10 = importlib.import_module("dex").build_top10
        DEX_MODULE_AVAILABLE = True
        _DEX_IMPORT_ERROR = None
        logging.getLogger(__name__).info(
            f"[LAZY_IMPORT] dex loaded on demand | RSS {_rss_before:.1f}MB -> {_process_rss_mb():.1f}MB"
        )
        return True
    except Exception as _dex_err:
        DEX_MODULE_AVAILABLE = False
        _DEX_IMPORT_ERROR = _dex_err
        logging.getLogger(__name__).error(f"DEX module lazy-load failed: {_dex_err}")
        return False

async def dex_build_top10(*a, **kw):
    if not _ensure_dex_loaded():
        raise ImportError(f"dex.py not available: {_DEX_IMPORT_ERROR}")
    return await _DEX_BUILD_TOP10(*a, **kw)

# ── Airdrops module — deferred import to reduce boot RSS/OOM ─────────────────
_AIRDROPS_HANDLERS = {}
AIRDROPS_MODULE_AVAILABLE = False
_AIRDROP_IMPORT_ERROR = None

def _ensure_airdrops_loaded() -> bool:
    global AIRDROPS_MODULE_AVAILABLE, _AIRDROP_IMPORT_ERROR
    if _AIRDROPS_HANDLERS:
        AIRDROPS_MODULE_AVAILABLE = True
        return True
    _rss_before = _process_rss_mb()
    try:
        _airdrops_mod = importlib.import_module("airdrops")
        _AIRDROPS_HANDLERS.update({
            "button": _airdrops_mod.handle_airdrops_button,
            "filter": _airdrops_mod.handle_airdrops_filter,
            "command": _airdrops_mod.handle_airdrops_command,
        })
        AIRDROPS_MODULE_AVAILABLE = True
        _AIRDROP_IMPORT_ERROR = None
        logging.getLogger(__name__).info(
            f"[LAZY_IMPORT] airdrops loaded on demand | RSS {_rss_before:.1f}MB -> {_process_rss_mb():.1f}MB"
        )
        return True
    except Exception as _ad_err:
        AIRDROPS_MODULE_AVAILABLE = False
        _AIRDROP_IMPORT_ERROR = _ad_err
        logging.getLogger(__name__).warning(f"[AIRDROP] airdrops.py lazy-load failed: {_ad_err}")
        return False

async def handle_airdrops_button(*a, **kw):
    if not _ensure_airdrops_loaded():
        return None
    return await _AIRDROPS_HANDLERS["button"](*a, **kw)

async def handle_airdrops_filter(*a, **kw):
    if not _ensure_airdrops_loaded():
        return None
    return await _AIRDROPS_HANDLERS["filter"](*a, **kw)

async def handle_airdrops_command(*a, **kw):
    if not _ensure_airdrops_loaded():
        return None
    return await _AIRDROPS_HANDLERS["command"](*a, **kw)

# ── Handler modules — deferred import to reduce boot RSS/OOM ─────────────────
_HANDLER_MODULES_LOADED = False
_HANDLER_MODULES_ERROR = None
_HANDLER_EXPORTS = {}
_FEATURES_TARGET_LABELS = {}
_COMPLEXITY_EMOJI = {}
_PRIORITY_EMOJI = {}

def _ensure_handler_modules_loaded() -> bool:
    global _HANDLER_MODULES_LOADED, _HANDLER_MODULES_ERROR
    global _FEATURES_TARGET_LABELS, _COMPLEXITY_EMOJI, _PRIORITY_EMOJI
    if _HANDLER_MODULES_LOADED:
        return True
    _rss_before = _process_rss_mb()
    try:
        _trending_mod = importlib.import_module("trending")
        _alpha_mod = importlib.import_module("alpha_signals")
        _cross_mod = importlib.import_module("cross")
        _sector_mod = importlib.import_module("sector_rotation")
        _ai_mod = importlib.import_module("ai_assistant")
        _features_mod = importlib.import_module("features")
        _feedback_mod = importlib.import_module("feedback")

        _FEATURES_TARGET_LABELS = getattr(_features_mod, "_FEATURES_TARGET_LABELS", {})
        _COMPLEXITY_EMOJI = getattr(_features_mod, "_COMPLEXITY_EMOJI", {})
        _PRIORITY_EMOJI = getattr(_features_mod, "_PRIORITY_EMOJI", {})

        _HANDLER_EXPORTS.update({
            "trending_coins": safe_menu_handler(_trending_mod.trending_coins),
            "alpha_signals": safe_menu_handler(_alpha_mod.alpha_signals),
            "cross_analysis": safe_menu_handler(_cross_mod.cross_analysis),
            "sector_rotation": safe_menu_handler(_sector_mod.sector_rotation),
            "ai_assistant": _ai_mod.ai_assistant,
            "ai_choice": safe_menu_handler(_ai_mod.ai_choice),
            "features_button_handler": safe_menu_handler(_features_mod.features_button_handler),
            "feedback_button_handler": safe_menu_handler(_feedback_mod.feedback_button_handler),
            "feedback_callback_handler": safe_menu_handler(_feedback_mod.feedback_callback_handler),
            "feedback_evidence_handler": _feedback_mod.feedback_evidence_handler,
            "get_feedback_inline_keyboard": _feedback_mod.get_feedback_inline_keyboard,
        })
        _HANDLER_MODULES_LOADED = True
        _HANDLER_MODULES_ERROR = None
        logging.getLogger(__name__).info(
            f"[LAZY_IMPORT] handler modules loaded on demand | RSS {_rss_before:.1f}MB -> {_process_rss_mb():.1f}MB"
        )
        return True
    except Exception as _hm_err:
        _HANDLER_MODULES_LOADED = False
        _HANDLER_MODULES_ERROR = _hm_err
        logging.getLogger(__name__).error(f"[HANDLER_MODULES] Lazy import failed: {_hm_err}")
        return False

async def _missing_handler(update, context):
    await update.message.reply_text("⚠️ Handler module not loaded.")

async def trending_coins(update, context):
    if not _ensure_handler_modules_loaded():
        return await _missing_handler(update, context)
    return await _HANDLER_EXPORTS["trending_coins"](update, context)

async def alpha_signals(update, context):
    if not _ensure_handler_modules_loaded():
        return await _missing_handler(update, context)
    return await _HANDLER_EXPORTS["alpha_signals"](update, context)

async def cross_analysis(update, context):
    if not _ensure_handler_modules_loaded():
        return await _missing_handler(update, context)
    return await _HANDLER_EXPORTS["cross_analysis"](update, context)

async def sector_rotation(update, context):
    if not _ensure_handler_modules_loaded():
        return await _missing_handler(update, context)
    return await _HANDLER_EXPORTS["sector_rotation"](update, context)

async def ai_assistant(update, context):
    if not _ensure_handler_modules_loaded():
        return await _missing_handler(update, context)
    return await _HANDLER_EXPORTS["ai_assistant"](update, context)

async def ai_choice(update, context):
    if not _ensure_handler_modules_loaded():
        return await _missing_handler(update, context)
    return await _HANDLER_EXPORTS["ai_choice"](update, context)

async def features_button_handler(update, context):
    if not _ensure_handler_modules_loaded():
        return await _missing_handler(update, context)
    return await _HANDLER_EXPORTS["features_button_handler"](update, context)

async def feedback_button_handler(update, context):
    if not _ensure_handler_modules_loaded():
        return await _missing_handler(update, context)
    return await _HANDLER_EXPORTS["feedback_button_handler"](update, context)

async def feedback_callback_handler(update, context):
    if not _ensure_handler_modules_loaded():
        return await _missing_handler(update, context)
    return await _HANDLER_EXPORTS["feedback_callback_handler"](update, context)

async def feedback_evidence_handler(u, c):
    if not _ensure_handler_modules_loaded():
        return False
    return await _HANDLER_EXPORTS["feedback_evidence_handler"](u, c)

def get_feedback_inline_keyboard():
    if not _ensure_handler_modules_loaded():
        return None
    return _HANDLER_EXPORTS["get_feedback_inline_keyboard"]()

# ── Technical Analysis module — deferred import to reduce boot RSS/OOM ──────
...
def run_ict_analysis(*a, **kw): raise ImportError("TechnicalAnalysis module not loaded yet")
...
class _TradeTrackerStub:
    ...
    def get_symbol_stats(self, symbol): return {}
_TRADE_TRACKER = _TradeTrackerStub()
_LAST_RESULT_CACHE = {}

def _ensure_ta_loaded() -> bool:
    global TA_MODULE_AVAILABLE, _TA_IMPORT_ERROR
    global run_ict_analysis, build_result_messages, generate_trade_chart
    global quick_check_markets, safe_edit, safe_send, fmt_price, log_analysis_event
    global compute_signal_probability, TV_FREE_SETUP_MSG, generate_tv_checklist
    global generate_tv_pine_script, get_current_session, get_silver_bullet_status
    global _LAST_RESULT_CACHE, _TRADE_TRACKER
    ...
    _rss_before = _process_rss_mb()
    try:
        _ta = importlib.import_module("TechnicalAnalysis_v9_6")
        run_ict_analysis             = _ta.run_ict_analysis
        build_result_messages        = _ta.build_result_messages
        generate_trade_chart         = _ta.generate_trade_chart
        quick_check_markets          = _ta.quick_check_markets
        safe_edit                    = _ta.safe_edit
        safe_send                    = _ta.safe_send
        fmt_price                    = _ta.fmt_price
        log_analysis_event           = _ta.log_analysis_event
        compute_signal_probability   = _ta.compute_signal_probability
        TV_FREE_SETUP_MSG            = _ta.TV_FREE_SETUP_MSG
        generate_tv_checklist        = _ta.generate_tv_checklist
        generate_tv_pine_script      = _ta.generate_tv_pine_script
        get_current_session          = _ta.get_current_session
        get_silver_bullet_status     = _ta.get_silver_bullet_status
        _LAST_RESULT_CACHE           = _ta._LAST_RESULT_CACHE
        _TRADE_TRACKER               = _ta._TRADE_TRACKER
        TA_MODULE_AVAILABLE = True
        _TA_IMPORT_ERROR = None
        logging.getLogger(__name__).info(
            f"[LAZY_IMPORT] TechnicalAnalysis_v9_6 loaded on demand | RSS {_rss_before:.1f}MB -> {_process_rss_mb():.1f}MB"
        )
        return True
    except Exception as _ta_err:
        TA_MODULE_AVAILABLE = False
        _TA_IMPORT_ERROR = _ta_err
        logging.getLogger(__name__).error(f"TechnicalAnalysis lazy-load failed: {_ta_err}")
        return False
...
def _process_rss_bytes() -> int:
    """Best-effort current RSS bytes for boot/import diagnostics."""
    try:
        with open("/proc/self/status", "r", encoding="utf-8", errors="ignore") as _f:
            for _line in _f:
...
def _log_rss(stage: str) -> None:
    logger.info(
        f"[RSS] {stage} | rss={_process_rss_mb():.1f}MB | "
        f"LOW_RAM_MODE={LOW_RAM_MODE} | LIGHT_START_MODE={LIGHT_START_MODE}"
    )


_log_rss("startup-imports-ready")
...
# State Management
background_tasks = {}
_user_menu_tasks: dict = {}  # [OOM-FIX] per-user active menu task — cancelled on every menu switch
regime_start_times = {}
user_ai_preference = {}
...
TA_QUEUE_ENABLED = os.getenv("TA_QUEUE_ENABLED", "1").strip().lower() not in {"0", "false", "no", "off"}
TA_QUEUE_MAX_PENDING = max(1, int(os.getenv("TA_QUEUE_MAX_PENDING", "1")))
TA_QUEUE_BUSY_NOTICE = os.getenv("TA_QUEUE_BUSY_NOTICE", "1").strip().lower() not in {"0", "false", "no", "off"}
_TA_JOB_QUEUE = TATaskQueue(max_pending=TA_QUEUE_MAX_PENDING, logger=logger)
...
def safe_menu_handler(fn):
    ...
    - [OOM-FIX] Cancels ANY running task from the previous menu immediately
    - [OOM-FIX] Cancels background auto_market_refresh loop for this user
    - Wraps handler as a tracked asyncio.Task so it can be cancelled by the next press
    ...
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # ── [OOM-FIX] Kill previous menu task for this user ──────────────────
        _prev_menu_task = _user_menu_tasks.pop(chat_id, None)
        if _prev_menu_task and not _prev_menu_task.done():
            _prev_menu_task.cancel()
            logger.info(f"[MENU_SWITCH] Cancelled previous menu task for chat_id={chat_id}")

        # ── [OOM-FIX] Kill background auto_market_refresh loop ───────────────
        _prev_bg_task = background_tasks.pop(chat_id, None)
        if _prev_bg_task and not _prev_bg_task.done():
            _prev_bg_task.cancel()
            logger.info(f"[MENU_SWITCH] Cancelled background refresh task for chat_id={chat_id}")

        # Yield to event loop so cancelled tasks actually stop, then free RAM
        ...
        # ── Wrap this handler as a tracked task so next menu press can kill it
        async def _run():
            try:
                await fn(update, context)
            except asyncio.CancelledError:
                logger.info(f"[MENU_SWITCH] {fn.__name__} cancelled mid-run for chat_id={chat_id}")
                raise
            except Exception as e:
        ...
        _user_menu_tasks[chat_id] = _task
        try:
            await _task
        except asyncio.CancelledError:
            pass  # another menu button was pressed — cancellation is expected
        finally:
            # Clean up our own entry if we finished normally (not cancelled by next press)
            _user_menu_tasks.pop(chat_id, None)

    return wrapper


# ── Handler modules are loaded lazily on first use — no boot import here ────
...
_cache_data = []
_cache_time = 0.0
_cache_lock = None   # [FIX] was never declared — run_cross_scan() uses `global _cache_lock` → NameError
...
async def run_cross_scan() -> list[CoinScore]:
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        # [CROSS-FIX] asyncio.gather(return_exceptions=True) puts Exception objects
        # directly into results list — not None. A raw Exception is truthy so
        # `if candles` passes, then candles.closes throws AttributeError → "Scan failed".
        # Fix: only process actual CandleData instances.
        all_signals: list[CrossInfo] = []
        for candles, (symbol, exchange, tf) in zip(results, meta):
            if isinstance(candles, CandleData) and len(candles.closes) >= MIN_CANDLES:
...
async def technical_analysis(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not TA_MODULE_AVAILABLE and not _ensure_ta_loaded():
        await update.message.reply_text(
            "❌ Technical Analysis module is not available.\n"
            f"Lazy-load error: `{_TA_IMPORT_ERROR}`",
            parse_mode="Markdown",
            reply_markup=create_main_keyboard()
        )
        return
...
async def _enqueue_ta_analysis(message, symbol: str, context: ContextTypes.DEFAULT_TYPE, market_type: str = "auto"):
    """Additive TA queue wrapper — keeps core TA pipeline unchanged and runs one job at a time."""
    if not TA_QUEUE_ENABLED:
        await _run_ta_analysis(message, symbol, context, market_type=market_type)
        return

    try:
        chat_id = getattr(message, "chat_id", None) or getattr(getattr(message, "chat", None), "id", 0) or 0
        status = await _TA_JOB_QUEUE.enqueue(
            TAJob(
                message=message,
                symbol=symbol,
                context=context,
        ...
        logger.error(f"[TA_QUEUE] enqueue failed for {symbol}: {_q_e}", exc_info=True)
        await message.reply_text(
            "⚠️ TA queue failed. Falling back to direct execution...",
            reply_markup=create_main_keyboard()
        )
        await _run_ta_analysis(message, symbol, context, market_type=market_type)
        return
    ...
    if not status.get("accepted"):
        await message.reply_text(
            "⏳ TA engine is busy right now. One analysis is running and one job is already queued.\n\n"
            "Please wait for the current job to finish, then try again.",
            reply_markup=create_main_keyboard()
        )
        return

    if TA_QUEUE_BUSY_NOTICE and (status.get("position", 0) > 1 or status.get("running")):
        await message.reply_text(
            f"⏳ *{symbol}* queued for Technical Analysis.\n"
            f"Queue position: *{status.get('position', 1)}*",
            parse_mode="Markdown",
            reply_markup=create_main_keyboard()
        )
...
async def _run_ta_analysis(message, symbol: str, context: ContextTypes.DEFAULT_TYPE, market_type: str = "auto"):
    ...
    if not TA_MODULE_AVAILABLE and not _ensure_ta_loaded():
        await message.reply_text(
            "❌ Technical Analysis module failed to load on demand.\n"
            f"Reason: `{_TA_IMPORT_ERROR}`",
            parse_mode="Markdown",
            reply_markup=create_main_keyboard()
        )
        return
...
async def ta_market_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.debug(f"[SILENT_EX] {type(_e).__name__}: {_e}")
    await _enqueue_ta_analysis(query.message, symbol, context, market_type=market_type)
...
async def _airdrop_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not _ensure_airdrops_loaded():
        await update.message.reply_text(
            "⚠️ Airdrops module is not available right now.",
            reply_markup=create_main_keyboard()
        )
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("ta_mode"):
        ...
        await _enqueue_ta_analysis(update.message, symbol, context)
        return
...
def main():
    ...
    async def _post_init(app):
        ...
        _log_rss("post_init-enter")
        if TA_QUEUE_ENABLED:
            await _TA_JOB_QUEUE.start(_run_ta_analysis)
            logger.info(f"[TA_QUEUE] started (max_pending={TA_QUEUE_MAX_PENDING})")
        ...
        if LOW_RAM_MODE:
            ...
            _log_rss("post_init-low-ram-tasks-started")
        else:
            ...
            _log_rss("post_init-full-tasks-started")
    application.post_init = _post_init

    async def _post_shutdown(app):
        """Cancel all tracked background tasks on clean shutdown (memory-safe for 127 MB)."""
        if TA_QUEUE_ENABLED:
            await _TA_JOB_QUEUE.stop()
            logger.info("[TA_QUEUE] stopped cleanly")
    ...
    # Keep registration unconditional; actual module import is deferred until first use.
    application.add_handler(CallbackQueryHandler(
        lambda u, c: handle_airdrops_filter(u, c, create_main_keyboard),
        pattern="^airdrop:"
    ))
    logger.info("[BOOT] Airdrop callback registered in deferred-import mode")
